...
The general function `naive_bayes()` automatically detects the class of the feature and assumes different distribution depending on the kind of the feature and the user choices. It currently supports following class conditional distributions: 
...
In addition to that specialized functions are available which implement specifically:
...
They will support sparse matrices in close future in order to boost the performance on the sparse data. Also few helper functions are provided that are supposed to improve the user experience. The `naive_bayes()` function is also available through the excellent `Caret` package. 
