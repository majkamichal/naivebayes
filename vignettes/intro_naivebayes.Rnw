...
%\VignetteIndexEntry{Introduction to the naivebayes package}
...
Each individual feature $X_i$ can take a value from a finite/infinte set of $m$ individually identified items (discrete feature) or it can be any real valued number (continuous feature). Discrete features are identified in \textcolor{darkgreen}{{\btt{naive\_bayes()}}} as variables of class "character", "factor", "logical" and "integer" when \textcolor{darkgreen}{{\btt{naive\_bayes(..., usepoisson = TRUE)}}}. On the other hand, continuous features are identified as variables with the class "numeric". Depending on the kind of the feature $X_i$, the \textcolor{darkgreen}{{\btt{naivebayes()}}} function uses a different probability distribution for modelling of the class conditional probability $\mathbb{P}(X_i = x_i | Y = C_k$) in the equation \eqref{eq:eq3}. In this subsection, available class conditional distributions are first introduced and then it is elaborated how they are assigned to the features.
...
$$\hat{p}_{ikl} = \frac{\sum_{j=1}^n \mathbb{1}(y^{(j)} = C_k \,\, and \,\, x_i^{(j)} = l)}{\sum_{j=1}^n \mathbb{1}(y^{(j)} = C_k)}$$

where $l \in \mathcal{X}_i$ and $\mathbb{1}$ is an indicator function that is 1 when the condition is satisfied and is 0 otherwise. Thus the Maximum-Likelihood yields very natural estimates: it is a ratio of the number of time the class label $C_k$ is observed together with the $l$-th value of the feature to the the number of times the class label $C_k$ is observed.

\subsubsection{Bayesian estimation and additive smoothing}

In case of the categorical distribution, applying the additive smoothing is commonly thought of as means of avoiding zero probabilities - adding a pseudocount $\alpha > 0$ to the number of observed cases in each category changes the expected probabilities and the resulting estimates are guaranteed to be non-zero. In the Bayesian realm, these estimates correspond to the expected value of the posterior distribution, when the symmetric Dirichlet distribution with the parameter $\boldsymbol{\alpha} = (\alpha,...,\alpha)$ is chosen as a prior. 

Let $\boldsymbol{X}_{ik} = \{X_{ik1},...,X_{ikn_k}\}$ represent independent samples for the $i$-th feature and $k$-th class with values in $M$ different categories $\mathcal{X}_i = \{item_1,...,item_M\}$. For simplicity, these categories are going to be refered to as $\{1,...,M\}$. Furthermore, let us define $\boldsymbol{p}_{ik} = (p_{ik1},...,p_{ikM})$ to be a vector containing the probabilities for the aforementioned categories for the same feature and class. Let us assume that the concentration hyperparameter $\boldsymbol{\alpha} = (\alpha,...,\alpha)$ consists of $M$ values, all being equal to $\alpha > 0$ and let  $\boldsymbol{c}_{ik} = (c_{ik1},...,c_{ikM})$ be a vector with number of counts in each category. Then the model can be represented as follows\footnote{Further details on the derivation of the posterior: \url{https://www.youtube.com/watch?v=UDVNyAp3T38} - this resource was chosen because it is very accessible and provides great explanations.}:

\vspace{3mm}

$\boldsymbol{p}_{ik}  \sim \text{ Dirichlet}(\boldsymbol{\alpha})$

\vspace{2mm}

$X_{ikj} | \boldsymbol{p}_{ik} \sim \text{ Categorical}(\boldsymbol{p}_{ik}), \,\,\,\,\,\, j = 1,...,n_k$

\vspace{2mm}

$\boldsymbol{p}_{ik} | \boldsymbol{X}_{ik} \sim \text{ Dirichlet}(\boldsymbol{c} + \boldsymbol{\alpha}) = \text{ Dirichlet}(c_{ik1} + \alpha, ..., c_{ikM} + \alpha)$

\vspace{3mm}

Since the Dirichlet distribution is the conjugate prior for the categorical distribution, the resulting posterior distribution is also Dirichlet. The parameter estimates can be obtained as the expected value of the posterior:

$$\hat{p}_{ikl} = \mathbb{E}(p_{ikl} | \boldsymbol{X}_{ik}) = \frac{c_{ikl} + \alpha}{\sum_{j=1}^M c_{ikj} + M\alpha}, \,\,\,\,\,\, l = 1,..., M.$$

When the parameter $\alpha = 0$ then each parameter estimate $\hat{p}_{ikl}$ coincide with the Maximum-Likelihood estimate and for $\alpha \rightarrow \infty$ they converge to $\{\frac{1}{M},...,\frac{1}{M} \}$. In other words the Maximum-Likelihood estimates are shrinked by adding pseudo-counts $\boldsymbol{\alpha}$ to counts $\boldsymbol{c}$. 

In this context, by using the symmetric Dirichlet prior we incorporate the prior information into the parameter estimation that the estimates cannot be zero but we have no preference to favour one parameter over any other. Also, the parameter $\alpha$ is usually set to $1$ because in such case the symmetric Dirichlet prior is equivalent to the uniform prior. The other popular value for $\alpha$ is 0.5 and in such case we end up with the non-informative Jeffreys prior.



\subsection{Poisson distribution}

The parameters of class conditional Poisson distributions, similarly as in case of the categorical distribution, can be estimated using either Maximum-Likelihood method or the Bayesian approach by adding pseudo-counts to the data. 
...
$$\hat{\lambda}_{ik} = \frac{\sum_{j=1}^n x_i^{(j)} \mathbb{1}(y^{(j)} = C_k) }{\sum_{j=1}^n \, \mathbb{1}(y^{(j)} = C_k)}.$$


\subsubsection{Bayesian estimation via pseudo-counts}

Assume that the training data that corresponds to the $i$-th feature is split into $K$ sub-samples according to the class labels $(C_1,...,C_K)$. Each sub-sample contains possibly different number of data points $n_k = \sum_{j=1}^n \mathbb{1}(y^{(j)} = C_k)$ and each sub-total is denoted by $T_k = \sum_{j=1}^n x_i^{(j)}\mathbb{1}(y^{(j)} = C_k)$. To one value in each sub-sample a pseudo-count $\alpha > 0$ is added. Consider now $k$-th sub-sample for which the average is taken to be the estimate for $\lambda_{ik}$:

$$\hat{\lambda}_{ik} = \frac{T_k + \alpha}{n_k}$$

The estimate $\hat{\lambda}_{ik}$ coincides with the expected value of posterior distribution given by Gamma$(T_k + \alpha, n_k)$, when the (degenerate) Gamma distribution with scale parameter $\alpha > 0$ and shape $\beta \rightarrow 0$ is chosen as a prior for the Poisson likelihood. 
...
The non-parametric estimate of the class conditional probability density function of a continuous feature $X_i$ can be obtained with a Kernel Density Estimator:

$$\hat{f}_{h_{ik}}(x) = \frac{1}{n_{k}h_{ik}}\sum_{j=1}^n\left(\frac{x - x_i^{(j)}}{h_{ik}}\right)\mathbb{1}(y^{(j)} = C_k),$$

where $n_k$ is number of samples in the $k$-th class, $K$ is a kernel function that defines the shape of the density curve and $h_{ik}$ is a class specific bandwidth that controls the smoothness. The estimation is performed using built in R function \textcolor{darkgreen}{{\btt{stats::density()}}}. Please see \textcolor{darkgreen}{{\btt{help(density)}}} and \textcolor{darkgreen}{{\btt{help(bw.nrd0)}}} for more details on available kernel functions and bandwidth selectors.
...
# Bayesian estimate Estimate for level1 has positive probability 
